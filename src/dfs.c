...
int dfs_coneccidad(int **grafo, int cantidad_nodos, int *ignorados) {
    ...
    if (visitados == NULL) {
        perror(ROJO "Error al reservar memoria para el arreglo de visitados\n" RESET_COLOR);
        return ERROR;
    }
    ...
    if (v == cantidad_nodos) {
        printf(ROJO "No se encontro un vertice inicial\n" RESET_COLOR);
        free(visitados);
        liberarPila(&pila);
        return ERROR;
    }
    ...
    if (!push(&pila, v)) {
        perror(ROJO "Error al insertar el elemento en la pila\n" RESET_COLOR);
        free(visitados);
        liberarPila(&pila);
        return ERROR;
    }
